...
do $check$ begin
 if (select count(*) from pg_roles where rolname in ('crm_h0_migration','crm_h0_runtime','crm_h0_untrusted') and rolcanlogin and not rolsuper and not rolcreatedb and not rolcreaterole and not rolinherit and not rolbypassrls and not rolreplication) <> 3 then raise exception 'HOSTED_ROLE_ATTRIBUTES_DENIED'; end if;
end $check$;
