...
-- Hosted operational variant:
--   * Supabase owns pgcrypto 1.3 in schema extensions.
--   * apply_migration supplies the outer transaction, so this file uses SET LOCAL ROLE.
-- The schema, ACL, RLS, function contracts, SQLSTATEs and data semantics are
-- otherwise byte-for-byte derived from the product migration.
do $$ begin
  if not exists (
    select 1
    from pg_extension e
    ...
    raise exception 'HOSTED_PGCRYPTO_MISMATCH';
  end if;
end $$;
...
begin
  ...
  if qf[13] = 'false' then
    ...
    if qf[14] !~ '^[a-z][a-z0-9-]{0,127}$' or qf[15] !~ '^[a-z][a-z0-9-]{0,127}$'
      ...
      raise exception 'F1_DENIED' using errcode='42501'; end if;
    expected_content := encode(extensions.digest(crm_f1.pack_fields(array[
  ...
  expected_material := encode(extensions.digest(crm_f1.pack_fields(array[
